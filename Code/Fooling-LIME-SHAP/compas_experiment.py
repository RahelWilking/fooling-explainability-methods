 ...
 * This may take some time because we iterate through every instance in the test set for
   all chosen methods and explanations take some time to compute
 ...
 * "Nothing shown" refers to a method yielding only 0 values
"""
import warnings
...
def experiment_main():
	"""
	Run through experiments for chosen methods on compas using both one and two unrelated features.
	...
	* We print out the rate at which features occur in the top three features
	...
	if "anchors" in models:
		print('---------------------')
		print("Beginning Anchors COMPAS Experiments....")
		print("(These take some time to run because we have to generate explanations for every point in the test set) ")
		print('---------------------')

		adv_anchors, anchors_explain = get_adv_model_and_exp_func("anchors", "compas", xtrain, ytrain, features, racist_model_f(),
																  innocuous_model_psi(),
	...
	if "explan" in models:
		print('---------------------')
		print("Beginning EXPLAN COMPAS Experiments....")
		print("(These take some time to run because we have to generate explanations for every point in the test set) ")
		print('---------------------')

		adv_explan, explan_explain = get_adv_model_and_exp_func("explan", "compas", xtrain, ytrain, features, racist_model_f(),
																innocuous_model_psi(),
	...
	if "lore" in models:
		print('---------------------')
		print("Beginning LORE COMPAS Experiments....")
		print("(These take some time to run because we have to generate explanations for every point in the test set) ")
		print('---------------------')

		adv_lore, lore_explain = get_adv_model_and_exp_func("lore", "compas", xtrain, ytrain, features, racist_model_f(),
															innocuous_model_psi(),
															categorical_features=categorical_features, pool=pool)
	...
	if "lime" in models:
		...
		adv_lime, lime_explain = get_adv_model_and_exp_func("lime", "compas", xtrain, ytrain, features, racist_model_f(),
															innocuous_model_psi(),
															categorical_features=categorical_features)
	...
	if "shap" in models:
		...
		adv_shap, shap_explain = get_adv_model_and_exp_func("shap", "compas", xtrain, ytrain, features, racist_model_f(),
															innocuous_model_psi(),
															categorical_features=categorical_features)
	...
	filename = "compas_experiment.csv"
