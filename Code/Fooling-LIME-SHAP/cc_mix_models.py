...
def experiment_main():
    ...
    for model in models:
        ...
        if model == "anchors":
            adv_model, explain_func = get_adv_model_and_exp_func(model, "cc", xtrain, ytrain, features,
                                                                racist_model_f(),
                                                                innocuous_model_psi(),
                                                                categorical_features=categorical_features)
        else:
            adv_model, explain_func = get_adv_model_and_exp_func(model, "cc", xtrain, ytrain, features, racist_model_f(),
                                                             innocuous_model_psi(),
    ...
    for model_adv in models:
        for model_exp in models:
            if model_exp == model_adv:
                continue
            print('---------------------')
            print("Beginning " + display_names[model_exp] + " explanations on " + display_names[model_adv] + " attack CC Experiments....")
            print(
