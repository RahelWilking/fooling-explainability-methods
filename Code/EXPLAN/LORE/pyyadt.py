...
def fit(df, class_name, columns, features_type, discrete, continuous,
    ...
    cmd = '../EXPLAN/LORE/yadt/dTcmd.exe -fd %s -fm %s -sep %s -d %s' % (
    ...
    try:
        ...
        print("has to sleep pe")
        time.sleep(60)
        dt = nx.DiGraph(nx.drawing.nx_pydot.read_dot(tree_filename))
    except TypeError as p:
        print("has to sleep te")
        time.sleep(60)
        dt = nx.DiGraph(nx.drawing.nx_pydot.read_dot(tree_filename))
    ...
    while not node_isleaf[node]:
        ...
        for child in dt.neighbors(node):
            ...
            if att in discrete:
                ...
                if yadt_value2type(edge_val, att, features_type) == val or math.isclose(yadt_value2type(edge_val, att, features_type),val,abs_tol=1e-6):
