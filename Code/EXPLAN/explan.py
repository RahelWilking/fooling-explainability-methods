...
def DataSelection(instance2explain, blackbox, dense_samples, tau):
    ...
    Changed to not fail if only one class is present.
    """
    ...
    for l in labels:
        ...
        while True:
            clustering = AgglomerativeClustering(n_clusters=n_clusters).fit(groups[-1])
            # Collecting data points belong to the cluster of instance2explain
            indices = np.where(clustering.labels_ == clustering.labels_[0])
            c_instance2explain = np.squeeze(groups[-1][indices, :], axis=0)
            # Checking the termination condition
            if c_instance2explain.shape[0] <= tau:
                break
            else:
                groups[-1] = c_instance2explain
    # Merging the representative samples of every class
    representative_samples = np.concatenate([np.array(groups[i]) for i in range(len(groups))])
    return representative_samples

# Data balancing step
def DataBalancing(blackbox, representative_samples, dataset):
    """
    ...
    labels = blackbox.predict(representative_samples)
    if(len(np.unique(labels))) > 1:
        balanced_samples, _ = oversampler.fit_sample(representative_samples, blackbox.predict(representative_samples))
        discrete_indices = dataset['discrete_indices']
        balanced_samples[:, discrete_indices] = np.around(balanced_samples[:, discrete_indices])
    else:
        balanced_samples = representative_samples
...
def InterpretabelModel(instance2explain, blackbox, neighborhood_data, dataset):
    ...
    # flip a random label (not 0 and 1, those are the instance to explain if this case happens)
    labels = dfX[class_name].unique()
    if len(labels) == 1:
        new_Label = 1 - labels[0]
        idx = np.random.choice(range(dfX.shape[0] - 2))
        dfX[class_name][idx+2] = new_Label

    # Using YaDT as interpretable model
    try:
        path = '../EXPLAN/LORE/yadt/tmp'+str(datetime.datetime.now().timestamp())+str(id(multiprocessing.current_process()))
        dt, dt_dot = pyyadt.fit(dfX, class_name, columns, features_type, discrete, continuous,
                            filename=dataset_name, path=path, sep=';', log=False)
